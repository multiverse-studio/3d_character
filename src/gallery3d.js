// src/gallery3d.js
// ----------------------------------------------------------------------------
// 3D Team Carousel (THREE.js) — versione completa con costanti rapide per LUCi + ENV
// ----------------------------------------------------------------------------
...
// ============================================================================
// QUICK SETTINGS — modifica qui per regolare luci e env map
// ============================================================================
const LIGHTS_ENABLED = true;

const HEMI_INTENSITY = 1.5;
const HEMI_SKY_COLOR = 0xffffff;
const HEMI_GROUND_COLOR = 0xb0b0b0;

const KEY_INTENSITY = 0.9;
const KEY_COLOR = 0xffffff;
const KEY_POS = [3, 5, 7];

const RIM_INTENSITY = 20;
const RIM_COLOR = 0xffffff;
const RIM_POS = [-7, 4, -5];

const FILL_INTENSITY = 0.5;
const FILL_COLOR = 0xffffff;
const FILL_POS = [-2, 2, 3];

// ENV map (Image Based Lighting)
const ENV_ENABLED = false;
const ENV_INTENSITY = 1.0;               // envMapIntensity sui materiali
const ENV_USE_HDR = false;
const ENV_HDR_PATH = '/hdrs/studio.hdr';
const ENV_BLUR = 0.03;              // blur RoomEnvironment

const DEBUG_BG_WHEN_DARK = false;            // sfondo nero se spegni tutto

// ============================================================================
// FLAGS UI
// ============================================================================
const SHOW_DESC = false; // true → mostra descrizione in topbar

// ============================================================================
// LIGHTS & ENV CONFIG (costruiti dalle costanti rapide)
// ============================================================================
const LIGHTS = {
  enabled: LIGHTS_ENABLED,
  hemi: { sky: HEMI_SKY_COLOR, ground: HEMI_GROUND_COLOR, intensity: HEMI_INTENSITY },
  key: { color: KEY_COLOR, intensity: KEY_INTENSITY, pos: KEY_POS },
  rim: { color: RIM_COLOR, intensity: RIM_INTENSITY, pos: RIM_POS },
  fill: { color: FILL_COLOR, intensity: FILL_INTENSITY, pos: FILL_POS },
};

const ENV = {
  enabled: ENV_ENABLED,
  useHDR: ENV_USE_HDR,
  hdrPath: ENV_HDR_PATH,
  blur: ENV_BLUR,
...
// ============================================================================
const CAMERA_CONFIG = {
  desktop: { position: [0.2, 0.5, 3.5], target: [0.0, 0, 0.0], fov: 45 },
  mobile: { position: [0.2, 1, 4.0], target: [0.0, 0.0, 0.0], fov: 45 },
...
// --- MOBILE TUNING ----------------------------------------------------------
const MOBILE_GEOM = { SIDE_MULT: 1.25, OFF_MULT: 1.6, OFF_Z_MULT: 1.2 };
const MOBILE_TIMING = { MOVE_TO_SIDE_MS: 520, OUT_FADE_MS: 360, IN_FADE_DELAY: 10 };
...
function getDynamicSpacing() {
  const t = clamp((window.innerWidth - CAMERA_CONFIG.MOBILE_MAX) / (1920 - CAMERA_CONFIG.MOBILE_MAX), 0, 1);
  return 1.0 + (1.5 - 1.0) * t;
}
function snapTransform(obj, pos, scale) { obj.position.copy(pos); obj.scale.setScalar(scale); }

// ============================================================================
// MATERIAL UTILITIES
// ============================================================================
function forceLitMaterials(root, { metalness = 0.0, roughness = 0.9 } = {}) {
  root.traverse(o => {
    ...
    arr.forEach((m, i) => {
      if (m.isMeshBasicMaterial) {
        ...
        nm.envMapIntensity = ENV.envMapIntensity;
        if (Array.isArray(o.material)) o.material[i] = nm; else o.material = nm;
      } else {
        if ('metalness' in m && (m.metalness == null)) m.metalness = metalness;
        if ('roughness' in m && (m.roughness == null)) m.roughness = roughness;
        if ('envMapIntensity' in m) m.envMapIntensity = ENV.envMapIntensity;
      }
      if ('emissiveIntensity' in m) m.emissiveIntensity = 0;
      if ('emissive' in m) m.emissive.set(0x000000);
      if (m.toneMapped === false) m.toneMapped = true;
    });
  });
}
function applyEnvIntensityToAll(root, value) {
  root.traverse(o => {
    if (!o.isMesh || !o.material) return;
...
export async function initGallery3D(canvasSelector) {
  ...
  const renderer = new THREE.WebGLRenderer({
    antialias: true, canvas, alpha: false, powerPreference: 'high-performance'
  });
  ...
  // LIGHTS + ENV (ottimizzate)
  let hemi = null, key = null, rim = null, fill = null;

  async function setupEnvironment() {
    if (!ENV.enabled) { scene.environment = null; return; }
    if (ENV.useHDR) {
      const rgbe = await new RGBELoader().loadAsync(ENV.hdrPath);
      const envTex = pmrem.fromEquirectangular(rgbe).texture;
      scene.environment = envTex;
      rgbe.dispose();
    } else {
      scene.environment = pmrem.fromScene(new RoomEnvironment(), ENV.blur).texture;
    }
  }
  function setupLights() {
    [hemi, key, rim, fill].forEach(L => { if (L && L.parent) L.parent.remove(L); });
    hemi = key = rim = fill = null;
    if (!LIGHTS.enabled) {
      if (DEBUG_BG_WHEN_DARK) scene.background = new THREE.Color(0x000000);
      return;
    }
    hemi = new THREE.HemisphereLight(LIGHTS.hemi.sky, LIGHTS.hemi.ground, LIGHTS.hemi.intensity); scene.add(hemi);
    key = new THREE.DirectionalLight(LIGHTS.key.color, LIGHTS.key.intensity); key.position.set(...LIGHTS.key.pos); scene.add(key);
    rim = new THREE.DirectionalLight(LIGHTS.rim.color, LIGHTS.rim.intensity); rim.position.set(...LIGHTS.rim.pos); scene.add(rim);
    fill = new THREE.DirectionalLight(LIGHTS.fill.color, LIGHTS.fill.intensity); fill.position.set(...LIGHTS.fill.pos); scene.add(fill);
  }
  async function setupLightsAndEnv() {
    await setupEnvironment();
    setupLights();
  }
  await setupLightsAndEnv();
  ...
  dracoLoader.setDecoderPath('/draco/gltf/'); // Path to decoder files
  ...
  // Load models (normalize, silhouette, materials)
  async function loadModel(entry) {
    ...
    // Pre-scale + normalize dimensione/centro
    const base = (entry.scale ?? 1) * SIZE_MULTIPLIER;
    original.scale.setScalar(base);
    original.updateMatrixWorld(true);
    const boxA = new THREE.Box3().setFromObject(original);
    const size = boxA.getSize(new THREE.Vector3());
    const maxAxis = Math.max(size.x, size.y, size.z) || 1;
    const targetScale = 1.4 / maxAxis;
    original.scale.multiplyScalar(targetScale);
    original.updateMatrixWorld(true);
    const boxB = new THREE.Box3().setFromObject(original);
    const center = boxB.getCenter(new THREE.Vector3());
    ...
    silhouette.traverse(o => {
      ...
      mm.depthWrite = false; mm.depthTest = true;
      o.renderOrder = 2; o.material = mm; matsSil.push(mm);
    });

    // Materiali originali (per opacità tween)
    const matsOrig = [];
    original.traverse(o => {
      if (o.isMesh && o.material) {
        (Array.isArray(o.material) ? o.material : [o.material]).forEach(m => {
          m.transparent = true; m.depthWrite = true; matsOrig.push(m);
        });
      }
    });
    ...
    // Allinea gruppo su X/Y per layout
    recenterGroup(root);

    // Material fixes/env intensity
    forceLitMaterials(original, { metalness: 0.0, roughness: 0.9 });
    killEmissive(original);
    applyEnvIntensityToAll(original, ENV.envMapIntensity);
  ...
  function setOpacities(root, origOpacity, silOpacity) {
    ...
    for (const m of (root.userData._matsOrig || [])) m.opacity = o;
    for (const m of (root.userData._matsSil || [])) m.opacity = s;
  }

  // Viewport config + depth
  function getViewportConfig() {
    if (isMobile()) return { spacing: SPACING_DESKTOP_BASE * 3.4, showSides: false, otherOpacity: 0.0 };
  ...
  function applySelectionMobile(index, first = false) {
    const N = roots.length; if (!N) return;
    ...
    roots.forEach((r) => {
      ...
      for (const m of (r.userData._matsSil || [])) m.opacity = 0;
    });
  ...
  function applySelectionDesktop(index, first = false) {
    ...
    roots.forEach((r, i) => {
      ...
      const isExiting = (newState === 'offL' || newState === 'offR') &&
  ...
  function tweenOpacities(root, targetOrig, targetSil, duration = CROSSFADE_MS, delay = 0, onDone = null) {
    ...
    const t0 = performance.now() + delay, t1 = t0 + duration;
    anims.add({ type: 'mats', obj: root, root, matsO, matsS, startO, startS, endO: targetOrig, endS: targetSil, t0, t1, onDone });
  }

  // Desktop exit: mantieni original(1.0) + silhouette(0.65) e poi spegni
  function exitWithSilhouette(r, fadeMs = FADE_OUT_MS, fadeDelay = FADE_OUT_DELAY_MS, keepSilMs = PHASE_KEEP_SIL_MS, onDone) {
    ...
    r.visible = true;
    setOpacities(r, 1.0, DESK_SIL_OPACITY);
    tweenOpacities(r, 0.0, 0.0, fadeMs, fadeDelay);
  ...
  function stepTweens(now) {
    for (const a of Array.from(anims)) {
      ...
      if (a.type === 'roty') a.obj.rotation.y = a.y0 + (a.y1 - a.y0) * e;
      else if (a.type === 'rotxz') { a.obj.rotation.x = a.x0 + (a.x1 - a.x0) * e; a.obj.rotation.z = a.z0 + (a.z1 - a.z0) * e; }
      else if (a.type === 'pos') a.obj.position.lerpVectors(a.start, a.target, e);
      else if (a.type === 'scale') a.obj.scale.setScalar(a.s0 + (a.s1 - a.s0) * e);
      else if (a.type === 'mats') {
        const { matsO, matsS, startO, startS, endO, endS } = a;
        for (let i = 0; i < matsO.length; i++) { matsO[i].opacity = startO[i] + (endO - startO[i]) * e; }
        for (let i = 0; i < matsS.length; i++) { matsS[i].opacity = startS[i] + (endS - startS[i]) * e; }
      }

      if (t >= 1) { const done = a.onDone; anims.delete(a); if (typeof done === 'function') done(); }
    }
  }
  ...
  function prewarmRenderer() {
    ...
    const prevVis = roots.map(r => r.visible);
    roots.forEach(r => r.visible = true);
    renderer.render(scene, camera);
    renderer.render(scene, camera);
    roots.forEach((r, i) => r.visible = prevVis[i]);
  }
  ...
  // Order: prewarm (canvas nascosto) → layout iniziale → loop
  ...
  function resizeRenderer() {
    ...
    const dprCap = (w <= CAMERA_CONFIG.MOBILE_MAX) ? 1.5 : 1.75;
